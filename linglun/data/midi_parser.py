"""
This implementation is based on paper Music Transformer and the code is refered to
https://github.com/jason9693/midi-neural-processor/blob/master/processor.py
"""
...
START_IDX = {
    "note_on": 1,
    "note_off": 1 + RANGE_NOTE_ON,
    "time_shift": 1 + RANGE_NOTE_ON + RANGE_NOTE_OFF,
    "velocity": 1 + RANGE_NOTE_ON + RANGE_NOTE_OFF + RANGE_TIME_SHIFT,
}


def read_midi_file(file_path):
    """
    Reads a MIDI file and returns a PrettyMIDI object.

    Args:
        file_path (str): Path to the MIDI file to be read.

    Returns:
        pretty_midi.PrettyMIDI: An object representing the MIDI file.
    """
...
class SustainAdapter:
    """
    Class to represent sustain pedal data.

    Attributes:
        start (float): The time when the sustain pedal action occurs.
        type (str): Type of sustain pedal action ('on' or 'off').
    """
...
class SustainDownManager:
    """
    Manages notes affected by the sustain pedal.

    Attributes:
        start (float): Start time of the sustain pedal being pressed.
        end (float): End time of the sustain pedal being pressed.
        managed_notes (list): Notes affected by the sustain pedal.
        _note_dict (dict): Dictionary with note pitch as key and note start time as
        value.
    """
...
class SplitNote:
    """
    Represents a divided note categorized by note_on and note_off events.

    Attributes:
        type (str): Type of note event ('note_on' or 'note_off').
        time (float): Time when the event occurs.
        velocity (int): Velocity of the note.
        value (int): Pitch value of the note.
    """
...
class Event:
    """
    Represents a musical event, which can be converted to an integer and vice versa.

    Attributes:
        type (str): Type of event.
        value (int): Value associated with the event.
    """
...
def _divide_note(notes):
    """the input sequences is by notes and the output is by note_on, note_off"""
    result_array = []
    ...
    for note in notes:
        ...
        # append on, off note to the array
        result_array += [on, off]
    return result_array
    ...
    for snote in snote_sequence:
        ...
        if snote.type == "note_on":
            ...
            try:
                ...
                result_array.append(pretty_midi_note)
            except:
...
def _note_preprocess(sustains, notes):
    """Processes notes based on sustain controls."""
    note_stream = []
...
def encode_midi(file=None, mid=None):
    if file:
        mid = pretty_midi.PrettyMIDI(midi_file=file)
    events = []
    notes = []

    # 1. if the midi file has sustain controls, process notes based on sustain controls
    for inst in mid.instruments:
        inst_notes = inst.notes
        # ctrl.number is the number of sustain control. If you want to know about the
        # number type of control,
        # see https://www.midi.org/specifications-old/item/table-3-control-change
        # -messages-data-bytes-2
        ctrls = _control_preprocess(
            [ctrl for ctrl in inst.control_changes if ctrl.number == 64]
        )
        notes += _note_preprocess(ctrls, inst_notes)

    # 2. divide notes into note_on, note_off
    ...
    return {"ids": [[e.to_int() for e in events]]}
...
def decode_midi(idx_array, file_path=None):
    ...
    # if want to change instrument, see
    # https://www.midi.org/specifications/item/gm-level-1-sound-set
    instrument = pretty_midi.Instrument(1, False)
